...
func TestAsyncDBM(t *testing.T) {
	...
	value_str, status := async.Get("two").GetStr()
	CheckEq(t, StatusSuccess, status)
	CheckEq(t, "step:2", value_str)
	...
	CheckEq(t, "*", dbm.GetSimple("num", "*"))
	inc_value, status := async.Increment("num", 2, 100).GetInt()
	CheckEq(t, StatusSuccess, status)
	CheckEq(t, 102, inc_value)
	inc_value, status = async.Increment("num", 3, 100).GetInt()
	CheckEq(t, StatusSuccess, status)
	CheckEq(t, 105, inc_value)
	...
	CheckEq(t, StatusSuccess, async.Rebuild("").Get())
	CheckEq(t, StatusSuccess, async.Synchronize(false, "").Get())
	CheckEq(t, StatusSuccess, async.CopyFileData(copyPath).Get())
	CheckEq(t, StatusSuccess, async.Clear().Get())
	CheckEq(t, 0, dbm.CountSimple())
	copyDBM := NewDBM()
	CheckEq(t, StatusSuccess, copyDBM.Open(copyPath, true, ""))
