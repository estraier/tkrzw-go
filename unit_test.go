...
func TestIndex(t *testing.T) {
	...
	values = index.GetValues("triple", 0)
	CheckEq(t, 1, len(values))
	CheckEq(t, "222", values[0])
	values = index.GetValues("foo", 0)
	CheckEq(t, 0, len(values))
	CheckTrue(t, index.Remove("single", "1"))
	CheckTrue(t, index.Remove("double", "11"))
	CheckFalse(t, index.Remove("triple", "x"))
	values = index.GetValues("double", 0)
	CheckEq(t, 1, len(values))
	CheckEq(t, "22", values[0])
