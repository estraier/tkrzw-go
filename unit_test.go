...
func TestIndex(t *testing.T) {
	tmpDir := MakeTempDir()
	defer os.RemoveAll(tmpDir)
	filePath := path.Join(tmpDir, "index.tkt")
	index := NewIndex()
	status := index.Open(filePath, true, ParseParams("truncate=true"))
	CheckEq(t, StatusSuccess, status)
	CheckTrue(t, len(index.String()) > 0)
	CheckFalse(t, index.Check("single", "1"))
	CheckFalse(t, index.Check("double", "11"))
	CheckTrue(t, index.Add("single", "1").IsOK())
	CheckTrue(t, index.Add("double", "11").IsOK())
	CheckTrue(t, index.Check("single", "1"))
	CheckTrue(t, index.Check("double", "11"))
	CheckTrue(t, index.Add("single", "2").IsOK())
	CheckTrue(t, index.Add("double", "22").IsOK())
	CheckTrue(t, index.Add("triple", "222").IsOK())
	values := index.GetValues("single", 0)
	CheckEq(t, 2, len(values))
	CheckEq(t, "1", values[0])
	CheckEq(t, "2", values[1])
	strValues := index.GetValuesStr("triple", 0)
	CheckEq(t, 1, len(strValues))
	CheckEq(t, "222", strValues[0])
	strValues = index.GetValuesStr("foo", 0)
	CheckEq(t, 0, len(strValues))
	CheckTrue(t, index.Remove("single", "1").IsOK())
	CheckTrue(t, index.Remove("double", "11").IsOK())
	CheckFalse(t, index.Remove("triple", "x").IsOK())
	strValues = index.GetValuesStr("double", 0)
	CheckEq(t, 1, len(strValues))
	CheckEq(t, "22", strValues[0])
	CheckEq(t, 3, index.Count())
	CheckEq(t, filePath, index.GetFilePath())
	CheckTrue(t, index.Synchronize(false).IsOK())
	CheckTrue(t, index.Rebuild().IsOK())
	CheckEq(t, 3, index.Count())
	...
	CheckTrue(t, index.Add("second", "22").IsOK())
	CheckTrue(t, index.Add("third", "333").IsOK())
	...
	CheckFalse(t, rv)
	iter.Last()
	strKey, strValue, rv := iter.GetStr()
	CheckTrue(t, rv)
	CheckEq(t, "third", strKey)
	CheckEq(t, "333", strValue)
	iter.Previous()
	strKey, strValue, rv = iter.GetStr()
	CheckTrue(t, rv)
	CheckEq(t, "second", strKey)
	CheckEq(t, "22", strValue)
	iter.Previous()
	strKey, strValue, rv = iter.GetStr()
	CheckEq(t, "first", strKey)
	CheckEq(t, "1", strValue)
	iter.Previous()
	strKey, strValue, rv = iter.GetStr()
	CheckFalse(t, rv)
	iter.Jump("second", "")
